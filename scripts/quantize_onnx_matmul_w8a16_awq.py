...
talker_quant_state.pt format (what the calibration step must produce)
--------------------------------------------------------------------
A ``torch.save`` of a dict (optionally nested under ``model_state_dict``). For
each layer ``li`` in 0..num_layers-1 and each op, THREE tensors are required:
    model.layers.{li}.self_attn.{q,k,v,o}_proj.weight                 [N, K] fp16/fp32
    model.layers.{li}.self_attn.{q,k,v,o}_proj.weight_quantizer._amax [N*num_groups, 1]
    model.layers.{li}.self_attn.{q,k,v,o}_proj.input_quantizer._pre_quant_scale [K]
    model.layers.{li}.mlp.{gate,up,down}_proj.weight / .weight_quantizer._amax / .input_quantizer._pre_quant_scale
i.e. the buffers ModelOpt writes after ``mtq.quantize(model, W8A16 AWQ config, calib_loader)``;
save with ``torch.save(model.state_dict(), "talker_quant_state.pt")``. Missing keys hard-fail.

EOS-bisection subsets (this fork's addition)
--------------------------------------------
    --quant-ops    all (default) | attn (q/k/v/o) | mlp (gate/up/down) | comma-list of op names
    --quant-layers inclusive layer range 'N-M' or single 'N' (default all)
Non-selected (layer, op) MatMuls stay FP16, so you can quantize a subset and check
whether EOS survives (handoff doc: full-W8A16 talker never emits EOS; bisection isolates
which MatMuls cause it).
"""
...
# Op subsets for EOS-bisection: quantize only attention vs only MLP MatMuls.
ATTN_OPS = ("q_proj", "k_proj", "v_proj", "o_proj")
MLP_OPS = ("gate_proj", "up_proj", "down_proj")
OP_GROUPS = {"all": LAYER_OP_ORDER, "attn": ATTN_OPS, "mlp": MLP_OPS}


def _parse_quant_selection(quant_ops: str, quant_layers: str | None, num_layers: int):
    """Return a predicate select(layer_idx, op_name) -> bool for which MatMul slots to quantize.

    --quant-ops: 'all' (default) | 'attn' (q/k/v/o) | 'mlp' (gate/up/down) | comma-list of op names.
    --quant-layers: inclusive layer range 'N-M' or single 'N' (default: all layers).
    Slots not selected are left FP16 (like the auto-skipped codec_head), enabling bisection.
    """
    key = quant_ops.strip().lower()
    if key in OP_GROUPS:
        ops = set(OP_GROUPS[key])
    else:
        ops = {o.strip() for o in quant_ops.split(",") if o.strip()}
        unknown = ops - set(LAYER_OP_ORDER)
        if unknown:
            raise ValueError(f"--quant-ops unknown op(s) {sorted(unknown)}; valid: {LAYER_OP_ORDER} or all/attn/mlp")
    if quant_layers:
        s = quant_layers.strip()
        lo, hi = (int(x) for x in s.split("-")) if "-" in s else (int(s), int(s))
    else:
        lo, hi = 0, num_layers - 1
    return lambda li, op: (lo <= li <= hi) and (op in ops)
...
def rewrite_model(
    ...
    *,
    ...
    for node in graph.node:
        ...
        if not select(layer_idx, op):
            # Bisection: this (layer, op) excluded by --quant-ops/--quant-layers — keep FP16.
            skipped_count += 1
            rewritten_nodes.append(node)
            continue
...
def main() -> None:
    ...
    parser.add_argument("--quant-ops", default="all",
                        help="EOS bisection: 'all'|'attn'(q/k/v/o)|'mlp'(gate/up/down)|comma-list of op names")
    parser.add_argument("--quant-layers", default=None,
                        help="EOS bisection: inclusive layer range 'N-M' or single 'N' (default: all)")
    ...
    select = _parse_quant_selection(args.quant_ops, args.quant_layers, args.num_layers)
    print(f"[awq-rewrite] subset select: quant-ops={args.quant_ops!r} quant-layers={args.quant_layers!r}")
