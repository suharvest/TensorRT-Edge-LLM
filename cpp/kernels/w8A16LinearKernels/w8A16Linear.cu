...
__global__ void w8a16_per_output_tiled_pair_k_kernel(
    half const* __restrict__ input, int8_t const* __restrict__ weight, half const* __restrict__ scales,
    half* __restrict__ output, int m, int n, int k)
{
    int const colBase = blockIdx.x * kOutputTile;
    ...
    if (outCol < n)
    {
        for (int kk = kLane * 2; kk < k; kk += kKThreads * 2)
        {
            float a = __half2float(input[inputOffset + kk]);
            float w = static_cast<float>(weight[kk * n + outCol]);
            sum += a * w;
            if (kk + 1 < k)
            {
                float a1 = __half2float(input[inputOffset + kk + 1]);
                float w1 = static_cast<float>(weight[(kk + 1) * n + outCol]);
                sum += a1 * w1;
            }
        }
    }
    ...
    int k, W8A16ScaleMode scaleMode, int groupSize, cudaStream_t stream) noexcept
{
    ...
    if (n >= kOutputTile)
    {
        ...
        static bool const usePairK = [] {
            char const* env = std::getenv("W8A16_PAIR_K");
            return env == nullptr || std::string(env) != "0";
        }();
        if (usePairK)
        {
            w8a16_per_output_tiled_pair_k_kernel<<<grid, kThreadsPerBlock, 0, stream>>>(
