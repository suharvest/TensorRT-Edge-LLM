...
bool StatefulCode2WavRunner::generateWaveform(
    std::vector<std::vector<int32_t>> const& codes, rt::audioUtils::AudioData& outputAudio, cudaStream_t stream)
{
    ...
    if (mMaxCodeLen <= 0)
    {
        LOG_ERROR("Invalid stateful Code2Wav max code length: %ld", mMaxCodeLen);
        return false;
    }
    ...
    reset(stream);
    for (int64_t start = 0; start < seqLen; start += mMaxCodeLen)
    {
        int64_t const end = std::min(start + mMaxCodeLen, seqLen);
        int64_t const currentFrames = end - start;
        std::vector<std::vector<int32_t>> chunkCodes(codes.size(), std::vector<int32_t>(currentFrames));
        for (size_t layer = 0; layer < codes.size(); ++layer)
        {
            std::copy(codes[layer].begin() + start, codes[layer].begin() + end, chunkCodes[layer].begin());
        }

        rt::audioUtils::AudioData chunkAudio;
        bool const isFinal = end == seqLen;
        if (!generateChunk(chunkCodes, isFinal, chunkAudio, stream))
        {
            LOG_ERROR("Stateful Code2Wav failed for frames [%ld, %ld)", start, end);
            return false;
        }
        if (!chunkAudio.waveform || chunkAudio.waveform->isEmpty())
        {
            LOG_ERROR("Stateful Code2Wav returned an empty waveform for frames [%ld, %ld)", start, end);
            return false;
        }
        if (chunkAudio.waveform->getDataType() != nvinfer1::DataType::kFLOAT)
        {
            LOG_ERROR("Stateful Code2Wav returned unsupported waveform dtype");
            return false;
        }

        int64_t const numDims = chunkAudio.waveform->getShape().getNumDims();
        int64_t const samples = chunkAudio.waveform->getShape()[numDims - 1];
        float const* chunkSamples = static_cast<float const*>(chunkAudio.waveform->rawPointer());
        waveformSamples.insert(waveformSamples.end(), chunkSamples, chunkSamples + samples);
    }

    outputAudio.waveform = std::make_shared<rt::Tensor>(
        rt::Tensor({1, static_cast<int64_t>(waveformSamples.size())}, rt::DeviceType::kCPU, nvinfer1::DataType::kFLOAT));
    std::memcpy(outputAudio.waveform->rawPointer(), waveformSamples.data(), waveformSamples.size() * sizeof(float));
    outputAudio.sampleRate = mConfig.sampleRate;
    outputAudio.numChannels = 1;
    outputAudio.hasWaveform = true;
    return true;
